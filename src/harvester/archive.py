...
def _year_dirs(raw_root: Path) -> list[Path]:
    ...
    Год здесь — год СКАЧИВАНИЯ, а не решения по делу (`RawStore.path_for`).
    Пока весь корпус скачан в 2026-м, каталог на суд один, и чистка идёт
    просто по судам по порядку; «старое скачанное первым» заработает,
    когда появится второй год. Главное, ради чего каталог — единица чистки,
    от этого не зависит: список ключей из бакета остаётся по одному суду,
    а не миллионом ключей в памяти.
    """
    dirs = [
        year
...
def prune(
    ...
    **подтверждённые запросом к бакету**, и никогда — записью в журнале.
    ...
    Чистка идёт от раньше скачанного к позже скачанному и останавливается,
    как только свободного места стало достаточно.
    """
    import shutil
