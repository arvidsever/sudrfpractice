...
def sweep_all(
    ...
    **И сам прогон ограничен по времени.** Не от зависаний: свод обязан
    ...
    def work(domain: str) -> None:
        empty_rounds = 0
        # Проба выключена 08.09.2026 по собственному правилу отмены.
        # Замер за двенадцать часов: суды и правда отвечают рано (52 %
        # за три минуты, 97 % за восемнадцать), но ответ мелкий — суд
        # почти сразу отказывает снова. Отказов на запрос стало 6,4 %
        # против 1,9 % без пробы, а выработка не выросла. Стучимся втрое
        # чаще ради того же результата — значит давим.
        #
        # `make_probe` оставлен: он проверен тестами и понадобится, если
        # пробовать реже (скажем, раз в десять минут). Это отдельный замер,
        # а не подкрутка на ходу.
        probe = None
