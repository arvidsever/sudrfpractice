...
* «Информация временно недоступна» — отдельный вердикт и повод остановиться,
  а не считать, что дел нет. Придержали ли нас или лежит модуль суда,
  по странице не различить: разметка одна и та же;
...
class Verdict(Enum):
    ...
    #: «Информация временно недоступна». Не ошибка запроса и не пустота:
    #: продолжать обход нельзя, надо отступить.
    #:
    #: Чем это вызвано — по странице НЕ видно. Платформа отдаёт одну и ту же
    #: разметку и когда придерживает адрес за частые запросы (2 КСОЮ 19.08,
    #: фикстура `ksoyu_temporarily_unavailable`), и когда у суда не работает
    #: сам модуль делопроизводства (3 КСОЮ, картотека `g3`: три захода
    #: 19.08 дали ответ до байта одинаковый, при том что остальные три
    #: картотеки того же суда в тот же день отвечали выдачей). Отступать
    #: надо в обоих случаях, но «суд нас придержал» — это прочтение,
    #: а не факт со страницы.
    THROTTLED = "throttled"
