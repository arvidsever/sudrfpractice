...
class HiddenActLinks(RuntimeError):
    """Под осью публикации у страницы нет ни одной ссылки на текст акта."""


def suspect_hidden_act_links(rows_with_links: int, total_rows: int) -> bool:
    """Признак того, что портал скрывает ссылки от нашего адреса.

    Под осью публикации ссылка обязана быть у КАЖДОЙ строки: дело попадает
    в выборку именно потому, что акт уже опубликован. Полная страница
    без единой ссылки означает не «ничего не опубликовано», а то, что мы
    смотрим на портал с адреса, которому он ссылки не показывает
    (проверено 19.08.2026 — см. docs/act-links-and-egress.md).
